...
const Navigation = () => {
    ...
    const expandedList = expandedObject.map((object) => {
        return (
            <Link className="dim link near-white pv2 f5 f5-l tl fw5 pl5 db"
                to={object.to}
                key={object.key}
                onClick={() => closeNav()}
            > {object.title} </Link>
    ...
    const originalList = expandedObject.map((object) => {
        return (
            <Link className="dim link dark-gray pa2 db tc bb b-dark-gray"
                to={object.to}
                key={object.key}
            > {object.title} </Link>
        )
    ...
    return (
        <nav className="dt w-100 pb4 mb1 response960">
            ...
            >
                ...
                <Link className={expandedAttributes}
                    style={expandedStyles}
                    to="/"
                    onClick={() => closeNav()}
                > 首頁 Home </Link>
                <div className="db dropdown">
                    <span className={expandedAttributes}
                        style={expandedStyles}
                    > 成員 Members
                            <b> </b>
                        <small className="dib fw7 light-green"
                            style={{ animation: "shiftDownAnimation 2s infinite" }}
                        > ▽ </small>
                    </span>
                    <div className="dn dropdown-content tc"
                        style={{ zIndex: "1" }}
                    > {expandedList} </div>
                </div>
            ...
            <div className="mw8 dib dtc-l v-mid w-100 w-75-l tr-l ph3">
                ...
                <div className="pr2 disappear960">
                    <Link className={originalAttributes}
                        to="/"
                    > 首頁 Home </Link>

                    <div className="dib relative dropdown">
                        <span className="link dark-gray f5 f5-l dib ph3 pb3"
                            style={{ cursor: "default" }}
                        > 成員 Members
                                <b> </b>
                            <small className="dib fw7 navy"
                                style={{ animation: "shiftDownAnimation 2s infinite" }}
                            >▽</small>
                        </span>
                        <div className="dn w-90 absolute dropdown-content tc br bl bt br1 b-dark-gray bg-white"
                            style={{ zIndex: "1" }}
                        > {originalList}
                        </div>
                    </div>
